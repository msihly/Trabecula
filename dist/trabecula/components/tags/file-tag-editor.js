...
import { Button, Comp, ConfirmModal, HeaderWrapper, Modal, TagInput, TagList, Text, UniformList, View, } from "trabecula/components";
...
export const FileTagEditor = Comp(({ batchId, fileIds }) => {
    ...
    const handleAdd = (tag) => {
        ...
        setRemovedTags((prev) => prev.find((t) => t.id === tag.id) ? prev.filter((t) => t.id !== tag.id) : prev);
        setHasUnsavedChanges(true);
    };
    const handleRemove = (tag) => {
        setAddedTags((prev) => prev.find((t) => t.id === tag.id) ? prev.filter((t) => t.id !== tag.id) : prev);
    ...
    return (<Modal.Container onClose={handleClose} isLoading={isLoading} maxWidth="50rem" width="100%" draggable>
      <Modal.Header>
      ...
      <Modal.Content dividers={false}>
        <UniformList row uniformWidth="20rem" height="15rem" spacing="0.5rem">
          <TagInput header="Tags to Add" value={addedTags} onChange={handleTagAdded} hasCreate hasDelete hasEditor autoFocus/>

          <HeaderWrapper header="Current Tags" height="100%">
            <TagList search={{ onChange: null, value: currentTags }} hasDelete={false} hasEditor hasInput rightNode={(tag) => (<View row>
                  <Button onClick={() => handleAdd(tag)} icon="Add" color="transparent" colorOnHover={colors.custom.blue} padding={{ all: "0.3em" }} boxShadow="none"/>

                  <Button onClick={() => handleRemove(tag)} icon="Close" color="transparent" colorOnHover={colors.custom.red} padding={{ all: "0.3em" }} boxShadow="none"/>
                </View>)}/>
          </HeaderWrapper>

          <TagInput header="Tags to Remove" value={removedTags} onChange={handleTagRemoved} includedIds={currentTags.map((t) => t.id)} hasDelete hasEditor/>
        </UniformList>
      </Modal.Content>
      ...
      {isConfirmDiscardOpen && (<ConfirmModal headerText="Discard Changes" subText="Are you sure you want to discard your changes?" confirmText="Discard" setVisible={setIsConfirmDiscardOpen} onConfirm={handleCloseForced}/>)}
    </Modal.Container>);
});
