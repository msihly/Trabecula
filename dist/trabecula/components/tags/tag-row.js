import { Comp, TagChip, View } from "trabecula/components";
...
export const TagRow = Comp((props) => {
    if (!props.tags?.length)
        return null;
    return (<View row spacing="0.5rem" overflow="auto hidden" {...props}>
      {sortTags(props.tags).map((tag) => (<TagChip key={tag.label} tag={tag} disabled={props.disabled} hasEditor/>))}
    </View>);
});
