...
import { Button, Card, CardGrid, Chip, Comp, Modal, MultiActionButton, Pagination, TagCard, TagFilterMenu, Text, UniformList, View, } from "trabecula/components";
import { useStores } from "trabecula/store";
import { colors, makeQueue, openSearchWindow, toast, useDeepEffect } from "trabecula/utils/client";
...
export const TagManager = Comp(() => {
    ...
    useDeepEffect(() => {
        if (resultsRef.current)
            resultsRef.current.scrollTo({ scrollTop: 0 });
    }, [store.results]);
    useEffect(() => {
        store.reset();
    ...
    const handlePageChange = (page) => store.loadFiltered({ page });
    ...
    const handleRegenerateRegEx = async () => {
        try {
            ...
            if (!regenRes.success)
                throw new Error(regenRes.error);
    ...
    return (<Modal.Container isLoading={stores.tag.manager.isLoading} onClose={handleClose} height="100%" width="100%">
      <Modal.Content dividers={false} padding={{ top: "1rem" }} overflow="hidden">
        <Card column flex={1} padding={{ all: 0 }} overflow="hidden" header={<UniformList row flex={1} justify="space-between">
              ...
              <View row justify="flex-end" spacing="0.5rem">
                <MultiActionButton name="Search" tooltip="Open Search Window with Selected Tags" onClick={handleSearchWindow} disabled={hasNoSelection}/>

                <MultiActionButton name="Label" tooltip="Edit Tag Relations" onClick={handleEditRelations} disabled={hasNoSelection}/>

                <MultiActionButton name="Refresh" tooltip="Refresh Selected Tags" onClick={handleRefreshTags} disabled={hasNoSelection}/>

                <MultiActionButton name="AccountTree" tooltip="Regenerate RegEx" onClick={handleRegenerateRegEx} disabled={hasNoSelection}/>

                <MultiActionButton name="Deselect" tooltip="Deselect All Tags" onClick={handleSelectNone} disabled={hasNoSelection}/>

                <MultiActionButton name="SelectAll" tooltip="Select All Tags in View" onClick={handleSelectAll}/>
              </View>
            </UniformList>}>
          <CardGrid cards={store.results.map((t) => (<TagCard key={t.id} tag={t}/>))}>
            <Pagination count={store.pageCount} page={store.page} isLoading={store.isPageCountLoading} onChange={handlePageChange} onFullLoad={handleFullPageLoad}/>
          </CardGrid>
        </Card>
      </Modal.Content>
