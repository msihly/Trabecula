...
import { Button, Card, Comp, Icon, IconButton, Input, MenuButton, Pagination, Text, View, } from "trabecula/components";
...
export const IconPicker = Comp(({ color = colors.custom.black, label = "Icon", menuProps = {}, setValue, value, viewProps = {}, width = "fit-content", withStylePicker = false, // Not used with icon font
...buttonProps }) => {
    ...
    return (<MenuButton button={renderButton} keepMounted={false} {...menuProps}>
        <View column padding={{ all: "0.5rem" }} spacing="0.5rem" overflow="auto" {...viewProps}>
          ...
          <Button text="No Icon" icon="Close" onClick={handleNoIcon} color={value === null ? colors.custom.black : colors.background} textColor={value === null ? colors.custom.white : colors.custom.lightGrey}/>

          <View row position="relative" spacing="0.5rem">
            {!withStylePicker ? null : (<Card column header="Style">
            ...
            <View column width="16rem" height="19rem">
              {chunkArray(pageIcons, 5).map((swatch, i) => (<View key={i} row>
                  {swatch.map((icon) => (<IconButton key={icon} name={icon} tooltip={icon} iconProps={{ size: "1.4em" }} sx={{
                    border: `3px solid ${value === icon ? colors.custom.white : "transparent"}`,
                }} onClick={() => setValue(icon)}/>))}
                </View>))}
            </View>

            <Pagination count={pageCount} page={page} onChange={setPage} siblingCount={0} boundaryCount={withStylePicker ? 1 : 0}/>
          </View>
        </View>
      </MenuButton>);
});
