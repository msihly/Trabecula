...
export const SideScroller = ({ children, className, innerClassName }) => {
    ...
    return (<View className={cx(css.root, className)}>
      <IconButton name="ChevronLeft" onClick={() => handleScroll("left")} className={cx(css.scrollButton, "left")} size="large"/>

      <View ref={ref} className={cx(css.items, innerClassName)}>
        {children}
      </View>

      <IconButton name="ChevronRight" onClick={() => handleScroll("right")} className={cx(css.scrollButton, "right")} size="large"/>
    </View>);
};
