...
export const setLogsPath = async (filePath) => {
...
const ensureStream = async () => {
    ...
    if (!initializing) {
        initializing = (async () => {
            if (!logsPath)
                await setLogsPath(await ipcRenderer.invoke("getLogsPath"));
...
/* ----------------------------------------------------------------------- */
export const fileLog = async (args, options) => {
    try {
        await ensureStream();
        if (!logStream)
            return;
...
/* ----------------------------------------------------------------------- */
export const makePerfLog = (logTag, toFile = false) => {
