...
export const createFiles = async (folder, fileDefs) => {
    try {
    ...
    for (const fileDef of fileDefs) {
        try {
            ...
            const file = await formatFile(`${makeSectionComment("THIS IS A GENERATED FILE. DO NOT EDIT.")}\n${await fileDef.makeFile()}`);
...
export const formatFile = (str) => prettier.format(str, { parser: "typescript", printWidth: 100, tabWidth: 2, useTabs: false });
export const makeIndexDef = (fileDefs) => {
    const imports = fileDefs.map((fileDef) => `export * from "./${fileDef.name}";`).join("\n");
    fileDefs.push({ name: "index", makeFile: async () => imports });
};
export const makeSectionComment = (sectionName) => `/* ${"-".repeat(75)} */\n/* ${" ".repeat(30)}${sectionName}\n/* ${"-".repeat(75)} */`;
export const parseExports = async (filePath) => {
...
export const parseExportsFromIndex = async (indexFilePath) => {
    ...
    const exportsMap = await Promise.all(fileNames.map((fileName) => parseExports(path.join(path.dirname(indexFilePath), `${fileName}.ts`))));
