...
export const bisectArrayChanges = (oldArr, newArr) => {
    if (!oldArr || !newArr)
        return { added: [], removed: [] };
    else
        return getArrayDiff(oldArr, newArr).reduce((acc, cur) => {
            acc[newArr.includes(cur) ? "added" : "removed"].push(cur);
            return acc;
        }, { added: [], removed: [] });
};
export const centeredSlice = (arr, indexToCenter, maxCount) => {
...
export const chunkArray = (arr, size) => [...Array(Math.ceil(arr.length / size))].map((_, i) => arr.slice(i * size, i * size + size));
...
export const objectToFloat32Array = (obj) => new Float32Array(Object.values(obj));
...
export const splitArray = (arr, filterFn) => arr.reduce((acc, cur) => (acc[+!filterFn(cur)].push(cur), acc), [[], []]);
export const sumArray = (arr, fn) => arr.reduce((acc, cur) => (acc += fn(cur)), 0);
